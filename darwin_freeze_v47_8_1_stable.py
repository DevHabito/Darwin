...
"""
DARWIN — Freeze Baseline v47.8.1 Stable

Congela o estado atual da v47.8.1 depois do seletor ampliado de micro-rotina.
...
Uso:
    py darwin_freeze_v47_8_1_stable.py --dry-run
    py darwin_freeze_v47_8_1_stable.py

Incluir logs, se houver:
    py darwin_freeze_v47_8_1_stable.py --include-logs
"""
...
OPTIONAL_FILES = [
    ...
    # v47.8 / v47.8.1
    "darwin_patch_v47_8_resolution_policy.py",
    ...
    "v47_8_resolution_policy_manifest.json",
    "v47_8_resolution_policy_repair_manifest.json",
...
README_TEXT = """DARWIN — Baseline v47.8.1 Stable
=================================

Esta baseline representa o marco estável da v47.8.1:
...
- painel 10c para dívida executiva ativa;
- micro-rotina de resolução de tensão;
- tabelas tension_resolution_routines e tension_resolution_steps;
- comando 10m para visualizar micro-rotina;
- seletor ampliado de política de micro-rotina;
- comando 10p para visualizar o estágio de reparo selecionado;
- repair v47.8.1 usando chr(10).join para evitar quebra de string.
...
começar v47.9/v48 a partir da pasta operacional atual, não desta baseline.

Direção natural:
transformar estágios classificados como compare/consolidate/archive/weaken em operadores reais,
mantendo segurança e reversibilidade.
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Congela baseline estável Darwin v47.8.1.")
    ...
    baseline_name = f"baseline_v47_8_1_stable_{now_stamp()}"
    baseline_dir = BASELINES_DIR / baseline_name

    print("=" * 72)
    print("DARWIN — FREEZE BASELINE v47.8.1 STABLE")
    print("=" * 72)
    ...
    if not args.dry_run:
        ...
        print()
        print(f"Baseline v47.8.1 congelada com sucesso em: {baseline_dir}")
        print(f"Pacote ZIP: {zip_path}")
        if warnings:
            print("ATENÇÃO: baseline contém registros de teste/tensão. Rode o purge antes se quiser uma baseline limpa.")
        else:
            print("Baseline limpa: sem tensões artificiais nem rotinas abertas.")
        print("Próximo passo: iniciar v47.9/v48 a partir da pasta operacional atual, não desta baseline.")
