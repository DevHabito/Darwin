...
"""
DARWIN — Freeze Baseline v47.13 Stable

Congela o estado atual da v47.13 depois da revisão de ciclos passados antes de agir.
...
Uso:
    py darwin_freeze_v47_13_stable.py --dry-run
    py darwin_freeze_v47_13_stable.py

Incluir logs, se houver:
    py darwin_freeze_v47_13_stable.py --include-logs
"""
...
OPTIONAL_FILES = [
    ...
    # v47.12/v47.12.1
    "darwin_patch_v47_12_cycle_report.py",
    ...
    "v47_12_cycle_report_manifest.json",
...
README_TEXT = """DARWIN — Baseline v47.13 Stable
================================

Esta baseline representa o marco estável da v47.13:
...
- influência contextual explícita na hipótese;
- hipótese com linhagem contextual;
- relatório consolidado do ciclo cognitivo;
- revisão de ciclos passados antes de agir;
- tabela tension_cycle_memory_reviews;
- comando 10y para visualizar a revisão de ciclos passados;
...
começar v47.14/v48 a partir da pasta operacional atual, não desta baseline.

Direção natural:
transformar a revisão de ciclos passados em seleção de estratégia:
quando um ciclo anterior foi bem-sucedido, Darwin deve escolher uma rotina semelhante;
quando foi malsucedido, deve marcar cautela, procurar variação ou formular uma hipótese alternativa.
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Congela baseline estável Darwin v47.13.")
    ...
    baseline_name = f"baseline_v47_13_stable_{now_stamp()}"
    baseline_dir = BASELINES_DIR / baseline_name

    print("=" * 72)
    print("DARWIN — FREEZE BASELINE v47.13 STABLE")
    print("=" * 72)
    ...
    if not args.dry_run:
        ...
        print()
        print(f"Baseline v47.13 congelada com sucesso em: {baseline_dir}")
        ...
        if warnings:
            ...
            print("Baseline limpa: sem tensões artificiais, rotinas, comparações, influências, linhagens, relatórios ou revisões pendentes.")
        print("Próximo passo: iniciar v47.14/v48 a partir da pasta operacional atual, não desta baseline.")
