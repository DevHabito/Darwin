...
"""
DARWIN — Freeze Baseline v47.11 Stable

Congela o estado atual da v47.11 depois da hipótese com linhagem contextual.
...
Uso:
    py darwin_freeze_v47_11_stable.py --dry-run
    py darwin_freeze_v47_11_stable.py

Incluir logs, se houver:
    py darwin_freeze_v47_11_stable.py --include-logs
"""
...
README_TEXT = """DARWIN — Baseline v47.11 Stable
================================

Esta baseline representa o marco estável da v47.11:
...
- tabela tension_hypothesis_lineage;
- comando 10h para visualizar a linhagem contextual da hipótese;
- ligação auditável entre hypothesis_id, comparison_id, influence_id, bias_label e confidence.
...
começar v47.12/v48 a partir da pasta operacional atual, não desta baseline.

Direção natural:
fazer a linhagem contextual influenciar a confiança operacional da hipótese
e/ou produzir um relatório consolidado do ciclo:
tensão → comparação → influência → hipótese → validação → fechamento.
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Congela baseline estável Darwin v47.11.")
    ...
    baseline_name = f"baseline_v47_11_stable_{now_stamp()}"
    baseline_dir = BASELINES_DIR / baseline_name

    print("=" * 72)
    print("DARWIN — FREEZE BASELINE v47.11 STABLE")
    print("=" * 72)
    ...
    if not args.dry_run:
        ...
        print()
        print(f"Baseline v47.11 congelada com sucesso em: {baseline_dir}")
        ...
        if warnings:
            ...
            print("Baseline limpa: sem tensões artificiais, rotinas abertas, comparações, influências ou linhagens pendentes.")
        print("Próximo passo: iniciar v47.12/v48 a partir da pasta operacional atual, não desta baseline.")
