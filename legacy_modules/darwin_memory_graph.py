...
def render_html(
    ...
    refresh_meta = f'<meta http-equiv="refresh" content="{int(refresh_seconds)}">' if refresh_seconds else ""
...
function initNetwork() {{
  ...
  const options = {{
    ...
    physics: {{
      ...
      barnesHut: {{ gravitationalConstant: -36000, springLength: 130, springConstant: 0.035, damping: 0.16 }}
    }},
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    refresh_seconds = max(1, int(args.interval)) if args.watch else None
