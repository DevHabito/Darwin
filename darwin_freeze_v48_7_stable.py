...
"""
DARWIN — Freeze Baseline v48.7 Stable

Congela o estado atual depois da transferência conceitual v48.7.
...
- v48.7: transferência conceitual para novos problemas, com explicação antes da ação.

Uso:
    py darwin_freeze_v48_7_stable.py --dry-run
    py darwin_freeze_v48_7_stable.py

Opcional:
    py darwin_freeze_v48_7_stable.py --include-logs
"""
...
README = """DARWIN — Baseline v48.7 Stable
==============================

Esta baseline representa o marco estável da v48.7.
...
Novo marco v48.7:
Darwin usou conceitos aprendidos na v48.6 para explicar antes de agir em
problemas novos. O ciclo auditável esperado é:

problema novo
→ recordar conceito
→ explicar antes da ação
→ decidir
→ agir
→ registrar resultado.

Conceitos transferidos:
- maior que → rejeitar por tamanho;
- dentro da tolerância → aceitar e inserir;
- profundo demais → rejeitar profundidade;
- ângulo desalinhado → rotacionar e inserir;
- forma diferente → rejeitar como forma, não como escala/orientação.

Cadeia pedagógica v48.7:
conceito aprendido → recordação contextual → explicação pré-ação → decisão operacional → execução segura.
...
começar v48.8 a partir da pasta operacional atual, não desta baseline.

Direção natural:
v48.8 — explicação causal contrastiva.
Darwin deve explicar não só “por que esta ação”, mas também “por que não outra”:
- por que rejeitar em vez de girar;
- por que girar em vez de rejeitar;
- por que tolerância permite inserir;
- por que forma diferente não é corrigível por escala/orientação.
"""
...
def live_v48_1(conn: sqlite3.Connection) -> dict[str, Any]:
    rs = rows(conn, "geometry_live_actions_v48_1")
...
def live_v48_6(conn: sqlite3.Connection) -> dict[str, Any]:
    ...
    if init:
        ...
        needed = ["size_hole", "size_large", "depth_hole", "depth_deep", "angle", "minimal_rotation"]
        values_varied = all(k in p for k in needed) and p["size_large"] > p["size_hole"] and p["depth_deep"] > p["depth_hole"]
...
def v48_7_indices(rs: list[dict[str, Any]], action: str, problem_id: str) -> list[int]:
    return [i for i, r in enumerate(rs) if r.get("action_kind") == action and r.get("problem_id") == problem_id]


def v48_7_ordered(rs: list[dict[str, Any]], problem_id: str) -> bool:
    needed = [
        v48_7_indices(rs, "problem_present", problem_id),
        v48_7_indices(rs, "concept_recall", problem_id),
        v48_7_indices(rs, "explanation_before_action", problem_id),
        v48_7_indices(rs, "action_decide", problem_id),
        v48_7_indices(rs, "action_execute", problem_id),
    ]
    if not all(needed):
        return False
    return needed[0][0] < needed[1][0] < needed[2][0] < needed[3][0] < needed[4][0]
...
def live_v48_7(conn: sqlite3.Connection) -> dict[str, Any]:
    ...
    checks = {
        ...
        "concept_recall_from_v48_6": any(
            r.get("action_kind") == "concept_recall"
            and r.get("payload", {}).get("source_table") == "geometry_measure_curriculum_v48_6"
            for r in rs
        ),
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Congela baseline estável Darwin v48.7.")
    ...
    bdir = BASELINES / f"baseline_v48_7_stable_{stamp()}"
    srcdir = bdir / "source_files"
    homedir = bdir / "darwin_home"

    manifest: dict[str, Any] = {
        "baseline": "v48.7_stable",
    ...
    print("=" * 72)
    print("DARWIN — FREEZE BASELINE v48.7 STABLE")
    print("=" * 72)
    ...
    live7 = summary.get("live_v48_7_summary", {})
    ...
    print(f"- v48_7_scenario_id: {live7.get('scenario_id')}")
    print(f"- v48_7_scenario_events_total: {live7.get('total')}")
    print(f"- v48_7_global_events_total: {live7.get('global_total')}")
    print(f"- v48_7_action_counts: {live7.get('counts')}")
    ...
    if not args.dry_run:
        ...
        print()
        print(f"Baseline v48.7 congelada com sucesso em: {bdir}")
        print(f"Pacote ZIP: {bdir.with_suffix('.zip')}")
        if warnings:
            print("ATENÇÃO: baseline tem avisos. Verifique antes de tratar como stable.")
        else:
            print("Baseline OK: v47 limpo, v48 pedagógico preservado e v48.7 transferência conceitual registrada.")
        print("Próximo passo: iniciar v48.8 a partir da pasta operacional atual, não desta baseline.")
