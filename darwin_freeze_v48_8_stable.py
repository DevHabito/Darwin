...
"""
DARWIN — Freeze Baseline v48.8 Stable

Congela o estado atual depois da explicação causal contrastiva v48.8.
...
- v48.8: explicação causal contrastiva, isto é, por que X e por que não Y.

Uso:
    py darwin_freeze_v48_8_stable.py --dry-run
    py darwin_freeze_v48_8_stable.py

Opcional:
    py darwin_freeze_v48_8_stable.py --include-logs
"""
...
README = """DARWIN — Baseline v48.8 Stable
==============================

Esta baseline representa o marco estável da v48.8.
...
Novo marco v48.8:
Darwin produziu explicações causais contrastivas antes de agir. Isso significa
não apenas explicar por que uma ação foi escolhida, mas também por que alternativas
foram rejeitadas.

Contrastes validados:
- rejeitar por tamanho em vez de girar;
- rejeitar por profundidade sem confundir com tamanho/rotação;
- girar em vez de rejeitar;
- inserir em vez de rejeitar quando está dentro da tolerância;
- rejeitar forma diferente em vez de tratar como rotação ou escala.

Cadeia pedagógica v48.8:
problema novo → avaliação causal → explicação contrastiva → rejeição de alternativas
→ decisão → execução segura.
...
começar v48.9 a partir da pasta operacional atual, não desta baseline.

Direção natural:
v48.9 — planejamento de ação em múltiplos passos.
Darwin deve escolher uma sequência curta de ações, justificar cada etapa e revisar
o plano se uma etapa falhar.
"""
...
def v48_7_has(rs: list[dict[str, Any]], action: str | None = None, problem_id: str | None = None,
              concept: str | None = None, relation: str | None = None,
              decision: str | None = None, outcome: str | None = None) -> bool:
    for r in rs:
        if action is not None and r.get("action_kind") != action:
            continue
        if problem_id is not None and r.get("problem_id") != problem_id:
            continue
        if concept is not None and r.get("recalled_concept") != concept:
            continue
...
def v48_7_ordered(rs: list[dict[str, Any]], problem_id: str) -> bool:
    chain = []
    for action in ["problem_present", "concept_recall", "explanation_before_action", "action_decide", "action_execute"]:
        items = [i for i, r in enumerate(rs) if r.get("action_kind") == action and r.get("problem_id") == problem_id]
        if not items:
            return False
        chain.append(items[0])
    return chain == sorted(chain)
...
def live_v48_7(conn: sqlite3.Connection) -> dict[str, Any]:
    ...
    checks = {
        "has_rows": bool(rs),
        "transfer_complete": v48_7_has(rs, "transfer_complete", outcome="success"),
        "all_problems_presented": all(v48_7_has(rs, "problem_present", p) for p in problems),
        "all_have_recall": all(v48_7_has(rs, "concept_recall", p) for p in problems),
        "all_explain_before_action": all(v48_7_ordered(rs, p) for p in problems),
        "oversize_explained": v48_7_has(rs, "explanation_before_action", "problem_oversize", "larger_smaller", "larger_than", "reject_size"),
        "tolerance_explained": v48_7_has(rs, "explanation_before_action", "problem_tolerance", "tolerance", "within_tolerance", "accept"),
        "depth_explained": v48_7_has(rs, "explanation_before_action", "problem_depth", "deep_shallow", "deeper_than", "reject_depth"),
        "rotation_explained": v48_7_has(rs, "explanation_before_action", "problem_rotation", "angle_rotation_minimum", "rotation_needed", "rotate"),
        "shape_explained": v48_7_has(rs, "explanation_before_action", "problem_shape", "shape_not_scale_or_orientation", "different_shape", "reject_shape"),
        "rotation_applied": v48_7_has(rs, "rotation_applied", "problem_rotation", outcome="success"),
        "rotation_insert_success": v48_7_has(rs, "insert_success", "problem_rotation", outcome="success"),
        "tolerance_insert_success": v48_7_has(rs, "insert_success", "problem_tolerance", outcome="success"),
        "safe_reject_oversize": v48_7_has(rs, "safe_reject", "problem_oversize", decision="reject_size"),
        "safe_reject_depth": v48_7_has(rs, "safe_reject", "problem_depth", decision="reject_depth"),
        "safe_reject_shape": v48_7_has(rs, "safe_reject", "problem_shape", decision="reject_shape"),
    }
    return {"scenario_id": sid, "total": len(rs), "global_total": len(all_rows), "counts": dict(counts), "checks": checks, "ready": all(checks.values())}


def v48_8_has(rs: list[dict[str, Any]], action: str | None = None, problem_id: str | None = None,
              relation: str | None = None, decision: str | None = None,
...
def v48_8_ordered(rs: list[dict[str, Any]], problem_id: str) -> bool:
    firsts = []
    for action in ["problem_present", "causal_assess", "contrastive_explanation", "alternative_rejected", "action_decide", "action_execute"]:
        items = [i for i, r in enumerate(rs) if r.get("action_kind") == action and r.get("problem_id") == problem_id]
        if not items:
            return False
...
def v48_8_alt_count(rs: list[dict[str, Any]], problem_id: str, n: int = 2) -> bool:
    return sum(1 for r in rs if r.get("action_kind") == "alternative_rejected" and r.get("problem_id") == problem_id) >= n
...
def live_v48_8(conn: sqlite3.Connection) -> dict[str, Any]:
    ...
    checks = {
        "has_rows": bool(rs),
        "contrastive_complete": v48_8_has(rs, "contrastive_complete", outcome="success"),
        "all_problems_presented": all(v48_8_has(rs, "problem_present", p) for p in problems),
        "all_have_contrastive_explanation": all(v48_8_has(rs, "contrastive_explanation", p) for p in problems),
        "all_have_alternative_rejections": all(v48_8_alt_count(rs, p, 2) for p in problems),
        "all_ordered": all(v48_8_ordered(rs, p) for p in problems),

        "reject_size_not_rotate": v48_8_has(rs, "contrastive_explanation", "contrast_oversize", "larger_than", "reject_size", "reject_size_not_rotate"),
        "oversize_rejected_rotate": v48_8_has(rs, "alternative_rejected", "contrast_oversize", alt="rotate"),
        "oversize_safe_reject": v48_8_has(rs, "safe_reject", "contrast_oversize", decision="reject_size"),

        "reject_depth_not_size_or_rotate": v48_8_has(rs, "contrastive_explanation", "contrast_depth", "deeper_than", "reject_depth", "reject_depth_not_size_or_rotate"),
        "depth_rejected_rotate": v48_8_has(rs, "alternative_rejected", "contrast_depth", alt="rotate"),
        "depth_safe_reject": v48_8_has(rs, "safe_reject", "contrast_depth", decision="reject_depth"),

        "rotate_not_reject": v48_8_has(rs, "contrastive_explanation", "contrast_rotation", "rotation_needed", "rotate", "rotate_not_reject"),
        "rotation_rejected_size": v48_8_has(rs, "alternative_rejected", "contrast_rotation", alt="reject_size"),
        "rotation_applied": v48_8_has(rs, "rotation_applied", "contrast_rotation", outcome="success"),
        "rotation_insert_success": v48_8_has(rs, "insert_success", "contrast_rotation", outcome="success"),

        "insert_not_reject": v48_8_has(rs, "contrastive_explanation", "contrast_insert", "within_tolerance", "accept", "insert_not_reject"),
        "insert_rejected_size": v48_8_has(rs, "alternative_rejected", "contrast_insert", alt="reject_size"),
        "insert_success": v48_8_has(rs, "insert_success", "contrast_insert", outcome="success"),

        "reject_shape_not_rotate_or_scale": v48_8_has(rs, "contrastive_explanation", "contrast_shape", "different_shape", "reject_shape", "reject_not_rotate_or_scale"),
        "shape_rejected_rotate": v48_8_has(rs, "alternative_rejected", "contrast_shape", alt="rotate"),
        "shape_rejected_scale": v48_8_has(rs, "alternative_rejected", "contrast_shape", alt="scale"),
        "shape_safe_reject": v48_8_has(rs, "safe_reject", "contrast_shape", decision="reject_shape"),

        "source_v48_7_available": any(
            r.get("action_kind") == "contrastive_init"
            and r.get("payload", {}).get("source_table") == "geometry_concept_transfer_v48_7"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Congela baseline estável Darwin v48.8.")
    ...
    bdir = BASELINES / f"baseline_v48_8_stable_{stamp()}"
    srcdir = bdir / "source_files"
    homedir = bdir / "darwin_home"

    manifest: dict[str, Any] = {
        "baseline": "v48.8_stable",
    ...
    print("=" * 72)
    print("DARWIN — FREEZE BASELINE v48.8 STABLE")
    print("=" * 72)
    ...
    live8 = summary.get("live_v48_8_summary", {})
    ...
    print(f"- v48_8_scenario_id: {live8.get('scenario_id')}")
    print(f"- v48_8_scenario_events_total: {live8.get('total')}")
    print(f"- v48_8_global_events_total: {live8.get('global_total')}")
    print(f"- v48_8_action_counts: {live8.get('counts')}")
    ...
    if not args.dry_run:
        ...
        print()
        print(f"Baseline v48.8 congelada com sucesso em: {bdir}")
        print(f"Pacote ZIP: {bdir.with_suffix('.zip')}")
        if warnings:
            print("ATENÇÃO: baseline tem avisos. Verifique antes de tratar como stable.")
        else:
            print("Baseline OK: v47 limpo, v48 pedagógico preservado e v48.8 explicação contrastiva registrada.")
        print("Próximo passo: iniciar v48.9 a partir da pasta operacional atual, não desta baseline.")
